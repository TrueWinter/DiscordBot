exports.run = async (client, message, args, level) => { // eslint-disable-line no-unused-vars
  function list () {
    ...
    tags.forEach((t) => {
      ...
      var c = (currentTag.tagID.split('-')[0] === message.guild.id);
      if (!c) return;
  ...
  switch (args[0]) {
    case 'add':
      ...
      client.tags.set(`${message.guild.id}-${tagName}`, `{"tagID": "${message.guild.id}-${tagName}", "content": "${content}", "addedBy": "${message.author.tag + ' (' + message.author.id + ')'}", "addedAt": "${new Date()}", "lastModified": "${new Date()}"}`);
