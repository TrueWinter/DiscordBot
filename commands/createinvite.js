exports.run = async (client, message, args, level) => { // eslint-disable-line no-unused-vars
  ...
  if(age.indexOf('s') !== -1) { // Seconds
    ...
    if (age.indexOf('m') !== -1) { // Minutes
      agemin = await age.replace(/m.*/, '');
      age = await agemin * 60;
    } else {
      if (age.indexOf('h') !== -1) { // Hours
        agehour = await age.replace(/h.*/, '');
        age = await agehour * 60 * 60;
      } else {
        if (age.indexOf('d') !== -1) { // Days
          ageday = await age.replace(/d.*/, '');
  ...
  const invite = message.channel.createInvite({maxUses: uses, maxAge: age}).then((invite) => {
    //console.log(invite);
